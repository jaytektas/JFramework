...
void test_scalars_and_type() {
    Variant n;                 assert(n.isNull() && !n.isValid());
    Variant b = true;          assert(b.isBool()   && b.type() == VariantType::Bool);
    Variant i = 42;            assert(i.isInt()    && i.toInt() == 42);
    Variant d = 3.5;           assert(i.isInt() && d.isDouble() && d.toDouble() == 3.5);
    Variant s = "torque";      assert(s.isString() && s.toString() == "torque");
    // bool/const char* pitfalls must not mis-resolve
    assert(Variant("x").isString());
    assert(Variant(0).isInt());
    assert(Variant(false).isBool());
    std::cout << "test_scalars_and_type passed\n";
}

void test_coercion() {
    assert(Variant("115200").toInt() == 115200);
    assert(Variant("3.14").toDouble() == 3.14);
    assert(Variant("true").toBool() == true);
    assert(Variant("off").toBool() == false);
    assert(Variant(1).toBool() == true);
    assert(Variant(42).toString() == "42");
    assert(Variant(true).toString() == "true");
    // tryValue does NOT coerce
    assert(Variant("5").tryValue<int>() == std::nullopt);
    assert(Variant(5).tryValue<int>() == 5);
    assert(Variant(5).tryValue<double>() == 5.0);   // int widens to double losslessly
    std::cout << "test_coercion passed\n";
}

void test_containers() {
    Variant l = Variant::list({1, "two", true});
...
void test_custom_and_equality() {
    struct Pt { int x, y; };
    Variant c = Variant::custom(Pt{3, 4});
    assert(c.isCustom());
    const Pt* p = c.customPtr<Pt>();
    assert(p && p->x == 3 && p->y == 4);
    assert(c.customPtr<int>() == nullptr);

    assert(Variant(5) == Variant(5));
    assert(Variant(5) == Variant(5.0));          // cross-numeric
    assert(Variant("a") != Variant("b"));
    assert(Variant() == Variant());
    assert(Variant::list({1, 2}) == Variant::list({1, 2}));
...
void test_property_bag() {
    ...
    int hits = 0;
    std::string lastKey;
    bag.onPropertyChanged.connect([&](std::string k, Variant) { ++hits; lastKey = k; });
...
void test_meta_class() {
    ...
    mc.field("port", &SerialConfig::port)
...
void test_json_bridge() {
    Variant v = fromJson(Json::parse(R"({"port":"/dev/ttyUSB0","baud":115200,"gain":3.14,"on":true,"tags":[1,2,3]})"));
    assert(v.isMap());
    ...
    Json j = toJson(v);
    Variant v2 = fromJson(Json::parse(j.dump()));
    assert(v2.at("baud").toInt() == 115200);
    assert(v2.at("tags").toList()[2].toInt() == 3);

    // Reflected object -> JSON.
    SerialConfig c{"/dev/ttyACM0", 57600, true};
    Json cj = toJson(Variant(MetaClass<SerialConfig>::instance().toMap(c)));
...
int main() {
    ...
    test_meta_class();
    test_json_bridge();
    std::cout << "All Variant/MetaObject tests passed.\n";
    return 0;
}
