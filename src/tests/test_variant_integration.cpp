...
// (2a) JTableModel numeric-aware sort (numeric strings sort by value, not lexically).
...
// (2b) Typed cells round-trip via the JVariant API.
...
// (2c) JSortFilterModel sorts numerically over typed cells (rebuild is synchronous).
...
// (2d) JTreeItem carries a typed JVariant payload.
void test_tree_typed_value() {
    JTreeModel t;
    t.insert("", {"run1", "Run 1"});
    assert(t.setValue("run1", 3.14));
    const JTreeItem* n = t.find("run1");
...
void test_object_serialization() {
    ...
    DeviceCfg b;
    deserialize(b, JJson::parse(j.dump()));
...
int main() {
    ...
    std::cout << "All JVariant integration tests passed.\n";
    return 0;
}
