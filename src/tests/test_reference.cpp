...
class FakeTable : public JWidget {
public:
    ...
    JVariant getRef(const std::string& key) const override {
        ...
        if (key == "yAxis")     return JVariant::list({500, 1000, 1500, 2000, 2500});
        if (key == "y")         return JVariant::map({{"sigid", 12}, {"n", 5}});
...
static JEvaluator::Resolver controlsResolver(const std::unordered_map<std::string, NodeId>& byName) {
    return [&byName](const std::string& n) -> JVariant {
        const size_t dot = n.find('.');
        const std::string head = (dot == std::string::npos) ? n : n.substr(0, dot);
        auto it = byName.find(head);
        if (it == byName.end()) return JVariant{};
...
void test_registry() {
    ...
    ev.registerResolver("config", [&](const std::string& n) -> JVariant {
        auto it = cfg.find(n);
        return it == cfg.end() ? JVariant{} : JVariant(it->second);
    }, /*priority*/ 5, /*sigil*/ '#');
...
void test_precompile() {
    ...
    JEvaluator ev;
    JEvaluator::JProgram p = ev.compile("[sld.value] * 100");
    assert(p.ok);
    s.setValue(0.25f);  assert(near(JEvaluator::run(p, R), 25.0));
    s.setValue(0.50f);  assert(near(JEvaluator::run(p, R), 50.0));   // same JProgram, fresh value
    ...
    JEvaluator::JProgram bad = ev.compile("1 + * 2");
    assert(!bad.ok);
...
// ---- thread-safety: one immutable JProgram, hammered from many threads --------
void test_threadsafe() {
    JEvaluator ev;
    JEvaluator::JProgram p = ev.compile("[%x] * 2 + 1");
    assert(p.ok);

    std::atomic<int> failures{0};
    auto worker = [&](double xv) {
        for (int i = 0; i < 50000; ++i) {
            double r = JEvaluator::run(p, [xv](const std::string&) { return JVariant(xv); });
...
//      per-call memo + cycle guard. The framework's JEvaluator knows nothing of it. ----
class DerivedStore {
public:
    explicit DerivedStore(const JEvaluator& ev) : m_ev(ev) {}
    ...
    double evaluate(const std::string& name, Ctx& ctx) const {
        ...
        double v = JEvaluator::run(d->second, [this, &ctx](const std::string& tok) -> JVariant {
            const std::string nm = (!tok.empty() && tok[0] == '#') ? tok.substr(1) : tok;
            double r = evaluate(nm, ctx);                              // recurse (framework reenters run)
            return std::isnan(r) ? JVariant{} : JVariant(r);
        });
...
private:
    const JEvaluator& m_ev;
    std::unordered_map<std::string, JEvaluator::JProgram> m_defs;
};
