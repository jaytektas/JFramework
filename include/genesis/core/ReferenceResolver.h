...
//
// Framework-free: it sits on Genesis::Variant + the Widget registry, nothing GUI-
...
inline Widget* widgetByNodeId(NodeId id) {
...
inline Variant applyIndexSuffix(Variant v, const std::string& seg, size_t lb) {
    size_t p = lb;
    while (p < seg.size() && seg[p] == '[') {
        const size_t rb = seg.find(']', p);
        if (rb == std::string::npos) return Variant{};
...
inline Variant resolveRefPath(Widget* root, const std::vector<std::string>& segs) {
    ...
    for (const std::string& seg : segs) {
        ...
        Variant v = !have ? root->getRef(key)
                          : (cur.isMap() ? cur.at(key) : Variant{});
        if (lb != std::string::npos)
            v = detail::applyIndexSuffix(v, seg, lb);
        cur = v;
        have = true;
    }
    return have ? cur : Variant{};
}
...
inline Variant resolveRef(const std::string& expr) {
    ...
    if (end == head.c_str() || *end != '\0') return Variant{};   // head wasn't a number
    Widget* w = widgetByNodeId(static_cast<NodeId>(id));
    if (!w) return Variant{};
