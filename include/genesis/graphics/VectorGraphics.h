...
// ============================================================================
// Genesis::VectorCanvas — anti-aliased 2D vector drawing.
//
...
// rounded rects) into per-vertex-coloured RenderVertex triangles, then hands
// them to a PrimitiveBuffer via pushGeometry(). The GPU "vector" pipeline draws
...
// Usage:
//   VectorCanvas vg;
//   vg.fillCircle(120, 120, 80, Paint::radial(120,120, 0, rgb(40,40,60),
//                                              80, rgb(10,10,20)));
...
inline Color withAlpha(Color c, uint8_t a) { c.a = a; return c; }
...
struct Paint {
    enum class Kind : uint8_t { Solid, Linear, Radial } kind{Kind::Solid};
    Color c0{}, c1{};
    float x0{0}, y0{0}, x1{0}, y1{0};   // linear: endpoints; radial: center + (r0,r1) in x1,y1
    float r0{0}, r1{0};

    Paint() = default;
    Paint(Color c) : kind(Kind::Solid), c0(c) {}

    static Paint solid(Color c) { return Paint(c); }
    static Paint linear(float x0, float y0, Color c0, float x1, float y1, Color c1) {
        Paint p; p.kind = Kind::Linear; p.x0 = x0; p.y0 = y0; p.c0 = c0;
        p.x1 = x1; p.y1 = y1; p.c1 = c1; return p;
    }
    static Paint radial(float cx, float cy, float rInner, Color cInner,
                        float rOuter, Color cOuter) {
        Paint p; p.kind = Kind::Radial; p.x0 = cx; p.y0 = cy;
...
enum class LineJoin : uint8_t { Miter, Bevel, Round };
...
public:
    ...
    explicit VectorCanvas(float aaWidth = 1.0f) : m_aa(aaWidth) {}

    void clear() { m_verts.clear(); m_subpaths.clear(); m_cur.clear(); }
    const std::vector<RenderVertex>& geometry() const { return m_verts; }
    bool empty() const { return m_verts.empty(); }

    // Emit accumulated geometry into a PrimitiveBuffer (does not clear).
    void flush(PrimitiveBuffer& buf) const {
    ...
    void stroke(float width, const Paint& paint,
    ...
    void drawLine(float x0, float y0, float x1, float y1, float width,
                  const Paint& paint, LineCap cap = LineCap::Butt) {
        std::vector<Vec2> pts{{x0, y0}, {x1, y1}};
        strokePolyline(pts, width, paint, false, cap, LineJoin::Miter);
    }
    void strokePolyline(const std::vector<Vec2>& pts, float width, const Paint& paint,
                        bool closed = false, LineCap cap = LineCap::Butt,
                        LineJoin join = LineJoin::Miter) {
        _strokePolyline(pts, width, paint, closed, cap, join);
    }
    void fillConvex(const std::vector<Vec2>& pts, const Paint& paint) {
        _fillConvexAA(pts, paint);
    }

    void fillRect(float x, float y, float w, float h, const Paint& paint) {
        fillConvex({{x, y}, {x + w, y}, {x + w, y + h}, {x, y + h}}, paint);
    }
    void strokeRect(float x, float y, float w, float h, float width, const Paint& paint) {
        strokePolyline({{x, y}, {x + w, y}, {x + w, y + h}, {x, y + h}}, width, paint, true);
    }
    void fillRoundedRect(float x, float y, float w, float h, float r, const Paint& paint) {
        fillConvex(roundedRectPoints(x, y, w, h, r), paint);
    }
    void strokeRoundedRect(float x, float y, float w, float h, float r, float width, const Paint& paint) {
    ...
    void fillEllipse(float cx, float cy, float rx, float ry, const Paint& paint) {
    ...
    void fillPie(float cx, float cy, float r, float a0, float a1, const Paint& paint) {
        std::vector<Vec2> pts;
        pts.push_back({cx, cy});
    ...
    // ===================== Geometry generators =====================
    static std::vector<Vec2> arcPoints(float cx, float cy, float r, float a0, float a1, int segs) {
        std::vector<Vec2> pts;
        float sweep = a1 - a0;
        if (segs <= 0) {
    ...
    static std::vector<Vec2> ellipsePoints(float cx, float cy, float rx, float ry, int segs) {
        if (segs <= 0) segs = static_cast<int>(std::ceil(2.f * kPi / (kPi / 24.f)));
        std::vector<Vec2> pts; pts.reserve(segs);
        for (int i = 0; i < segs; ++i) {
    ...
    static std::vector<Vec2> roundedRectPoints(float x, float y, float w, float h, float r) {
        float rr = r;
...
private:
    ...
    struct CV { Vec2 p; Paint paint; };  // a vertex carrying its paint for colour eval

    float m_aa{1.0f};
    bool  m_closed{false};
    std::vector<RenderVertex> m_verts;
    std::vector<std::vector<Vec2>> m_subpaths;
    std::vector<Vec2> m_cur;

    void _flushSub() {
        if (!m_cur.empty()) { m_subpaths.push_back(m_cur); m_cur.clear(); }
    }

    static Vec2 _sub(Vec2 a, Vec2 b) { return {a.x - b.x, a.y - b.y}; }
    static float _len(Vec2 a) { return std::sqrt(a.x * a.x + a.y * a.y); }
    static Vec2 _norm(Vec2 a) { float l = _len(a); return l > 1e-6f ? Vec2{a.x / l, a.y / l} : Vec2{0, 0}; }
    static Vec2 _perp(Vec2 d) { return {-d.y, d.x}; }  // left normal of a direction

    RenderVertex _mk(Vec2 p, Color c) {
        RenderVertex v;
    ...
    void _tri(RenderVertex a, RenderVertex b, RenderVertex c) {
    ...
    void _quad(CV a, CV b, CV c, CV d) {
        RenderVertex va = _mk(a.p, a.paint.eval(a.p.x, a.p.y));
        RenderVertex vb = _mk(b.p, b.paint.eval(b.p.x, b.p.y));
        RenderVertex vc = _mk(c.p, c.paint.eval(c.p.x, c.p.y));
        RenderVertex vd = _mk(d.p, d.paint.eval(d.p.x, d.p.y));
        _tri(va, vb, vc); _tri(va, vc, vd);
    }
    // Quad with explicit colours (used for fringe alpha fades).
    void _quadC(Vec2 a, Color ca, Vec2 b, Color cb, Vec2 c, Color cc, Vec2 d, Color cd) {
        _tri(_mk(a, ca), _mk(b, cb), _mk(c, cc));
        _tri(_mk(a, ca), _mk(c, cc), _mk(d, cd));
    }

    static Vec2 _centroid(const std::vector<Vec2>& pts) {
        Vec2 c{0, 0};
    ...
    void _fillConvexAA(const std::vector<Vec2>& pts, const Paint& paint, bool forceConvex = false) {
        (void)forceConvex;
        if (pts.size() < 3) return;
        // Interior fan.
        RenderVertex v0 = _mk(pts[0], paint.eval(pts[0].x, pts[0].y));
        ...
        if (m_aa <= 0.f) return;
        Vec2 ctr = _centroid(pts);
        size_t n = pts.size();
        for (size_t i = 0; i < n; ++i) {
            Vec2 a = pts[i], b = pts[(i + 1) % n];
            Vec2 dir = _norm(_sub(b, a));
            Vec2 nrm = _perp(dir);
            ...
            Vec2 ao{a.x + nrm.x * m_aa, a.y + nrm.y * m_aa};
            Vec2 bo{b.x + nrm.x * m_aa, b.y + nrm.y * m_aa};
            Color ca = paint.eval(a.x, a.y), cb = paint.eval(b.x, b.y);
            _quadC(a, ca, b, cb, bo, withAlpha(cb, 0), ao, withAlpha(ca, 0));
        }
    }

    // Stroke a polyline with miter joins + caps + outward fringe on both sides.
    void _strokePolyline(const std::vector<Vec2>& inPts, float width, const Paint& paint,
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            Vec2 nPrev = _perp(dPrev), nNext = _perp(dNext);
            Vec2 m{nPrev.x + nNext.x, nPrev.y + nNext.y};
            float ml = _len(m);
            Vec2 mn = ml > 1e-4f ? Vec2{m.x / ml, m.y / ml} : nNext;
        ...
        if (m_aa > 0.f) {
            for (size_t i = 0; i + 1 < n; ++i) {
                Vec2 ld = _norm(_perp(_norm(_sub(left[i + 1], left[i]))));  // outward-ish on left
                // left boundary outward = away from centerline (use left-right direction)
                Vec2 outL = _norm(_sub(left[i], pts[i]));
                Vec2 outL2 = _norm(_sub(left[i + 1], pts[i + 1]));
                (void)ld;
                Color c0 = paint.eval(left[i].x, left[i].y);
                Color c1 = paint.eval(left[i + 1].x, left[i + 1].y);
                Vec2 lo0{left[i].x + outL.x * m_aa, left[i].y + outL.y * m_aa};
                Vec2 lo1{left[i + 1].x + outL2.x * m_aa, left[i + 1].y + outL2.y * m_aa};
                _quadC(left[i], c0, left[i + 1], c1, lo1, withAlpha(c1, 0), lo0, withAlpha(c0, 0));

                Vec2 outR = _norm(_sub(right[i], pts[i]));
                Vec2 outR2 = _norm(_sub(right[i + 1], pts[i + 1]));
                Color r0 = paint.eval(right[i].x, right[i].y);
                Color r1 = paint.eval(right[i + 1].x, right[i + 1].y);
                Vec2 ro0{right[i].x + outR.x * m_aa, right[i].y + outR.y * m_aa};
                Vec2 ro1{right[i + 1].x + outR2.x * m_aa, right[i + 1].y + outR2.y * m_aa};
    ...
    static int _bezierSegments(Vec2 p0, Vec2 c1, Vec2 c2, Vec2 p3) {
