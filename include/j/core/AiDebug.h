...
// ============================================================================
// JAiDebug — AI bus introspection and debugging utilities
//
...
public:
    ...
    static std::string describeNode(const JWidget& w) {
    ...
    static void printNode(const JWidget& w) {
        std::cout << "[JAiDebug] " << describeNode(w) << '\n';
    }
    ...
    static std::string dumpSemanticTree() {
        ...
        out << "═══ Genesis Semantic Tree ("
            << JWidget::s_activeWidgets.size() << " widgets";
        if (!JDockWidget::s_activeDocks.empty())
            out << " + " << JDockWidget::s_activeDocks.size() << " docks";
        out << ") ═══\n";
        out << std::left
            << std::setw(18) << "Role"
            << std::setw(22) << "JLabel"
        ...
        for (auto* d : JDockWidget::s_activeDocks) {
            if (!d) continue;
    ...
    static std::string dumpBusSnapshot(const JSharedBusMemory* region) {
        if (!region) return "(null region)\n";
        std::vector<JAiNodeDescriptor> nodes;
        uint64_t frame = 0;
        if (!JAiControlBus::snapshot(region, nodes, &frame))
    ...
    static void printBusSnapshot(const JSharedBusMemory* region) {
        std::cout << dumpBusSnapshot(region) << std::flush;
    }

    // ── JAction trace — log and dispatch a semantic action ──────────────────
    ...
    static bool executeAndTrace(uint32_t nodeId, const std::string& action) {
        for (auto* w : JWidget::s_activeWidgets) {
            ...
            std::cout << "[JAiDebug] action \"" << action
                      << "\" on node " << nodeId
    ...
    static JWidget* findByLabel(const std::string& label) {
        for (auto* w : JWidget::s_activeWidgets) {
    ...
    static JWidget* findByRole(const std::string& role) {
        for (auto* w : JWidget::s_activeWidgets) {
    ...
    static std::vector<JWidget*> findAllByRole(const std::string& role) {
...
private:
    ...
    static std::string _flags(const JWidget& w) {
        std::string f;
        f += w.isEnabled()  ? 'E' : '-';
        f += w.isVisible()  ? 'V' : '-';
        f += w.isFocused()  ? 'F' : '-';
        f += (w.getState() == JWidgetState::Pressed) ? 'P' : '-';
