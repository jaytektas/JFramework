...
//
// message / confirm / input are Genesis-native overlays rendered inside the
// compositor — zero OS dependencies, consistent look across platforms.
//
// openFile / saveFile / openFolder enqueue a file-kind JDialogRequest; the app
// window turns it into a framework-rendered JFileDialogWindow (no zenity, no
// native picker, no platform types leaked here).
//
...
struct JDialogRequest {
    enum class JKind { Message, Confirm, Input, OpenFile, SaveFile, OpenFolder };
...
public:
    ...
    // ---- Open file — framework-rendered picker (JFileDialogWindow) ---------
    static void openFile(const std::string& title,
                         std::vector<std::string> extensions,
                         std::function<void(std::string)> onAccept,
                         std::function<void()>            onCancel = {}) {
        _pushFileRequest(JDialogRequest::JKind::OpenFile, title,
                         std::move(extensions), std::move(onAccept), std::move(onCancel));
    }

    // ---- Save file — framework-rendered picker -----------------------------
    static void saveFile(const std::string& title,
                         std::vector<std::string> extensions,
                         std::function<void(std::string)> onAccept,
                         std::function<void()>            onCancel = {}) {
        _pushFileRequest(JDialogRequest::JKind::SaveFile, title,
                         std::move(extensions), std::move(onAccept), std::move(onCancel));
    }

    // ---- Open directory — framework-rendered picker ------------------------
