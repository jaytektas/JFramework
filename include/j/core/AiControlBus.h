...
constexpr char     DefaultBusName[]  = "/jframework_ai_bus";
 ...
 */
struct alignas(16) JAiNodeDescriptor {
 ...
 */
struct alignas(16) JAiVirtualInput {
 ...
 */
struct alignas(16) JAiActionRequest {
 ...
 */
struct alignas(16) JAiSignalNotification {
 ...
 */
struct alignas(64) JSharedBusMemory {
    ...
    uint32_t nodeCount;
    uint32_t _reserved;
    JAiNodeDescriptor nodes[MaxBusNodes];
    JAiVirtualInput  inboundCommand;   // legacy pixel/key channel
    JAiActionRequest inboundAction;    // semantic act-by-id channel
    JAiSignalNotification outboundSignal; // outbound event notification channel
};
...
public:
    ...
    bool createSegment(const char* name = DefaultBusName) {
        ...
        if (ftruncate(fd, static_cast<off_t>(sizeof(JSharedBusMemory))) != 0) {
            ::close(fd); shm_unlink(name); return false;
        }
        void* p = mmap(nullptr, sizeof(JSharedBusMemory),
                       PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        ::close(fd);
        if (p == MAP_FAILED) { shm_unlink(name); return false; }
        std::memset(p, 0, sizeof(JSharedBusMemory));  // zero-fill => valid atomics(0)
        m_mapped = p;
        m_owner  = true;
        std::strncpy(m_name, name, sizeof(m_name) - 1);
        attach(p);
        qCInfo(LogAiBus) << "AI JControl Bus shared segment '" << name << "' created ("
                         << sizeof(JSharedBusMemory) << " bytes)." << std::endl;
        return true;
#else
        (void)name;
        return false;
    ...
    void attach(void* memorySegment) {
        ...
        m_sharedRegion = reinterpret_cast<JSharedBusMemory*>(memorySegment);
    ...
    void detach() {
#if defined(__linux__) || defined(__APPLE__)
        if (m_mapped) { munmap(m_mapped, sizeof(JSharedBusMemory)); m_mapped = nullptr; }
     ...
     */
    void publishNodes(const JAiNodeDescriptor* src, uint32_t count) {
        if (!m_sharedRegion) return;
        JSharedBusMemory& r = *m_sharedRegion;
        if (count > MaxBusNodes) count = MaxBusNodes;

        r.generation.fetch_add(1, std::memory_order_acq_rel);   // -> odd: publish begins
        r.nodeCount = count;
        if (count) std::memcpy(r.nodes, src, count * sizeof(JAiNodeDescriptor));
     ...
     */
    static bool snapshot(const JSharedBusMemory* r,
                         std::vector<JAiNodeDescriptor>& out,
        ...
        for (int tries = 0; tries < 256; ++tries) {
            ...
            out.resize(n);
            if (n) std::memcpy(out.data(), r->nodes, n * sizeof(JAiNodeDescriptor));
     ...
     */
    bool pollInboundCommand(JAiVirtualInput& outCommand) {
        ...
        if (m_sharedRegion->inboundCommand.type.load(std::memory_order_relaxed) ==
            JAiVirtualInput::JCommandType::None) return false;
     ...
     */
    static int submitActionBlocking(JSharedBusMemory* r, uint32_t targetId,
    ...
    JSharedBusMemory* region() noexcept { return m_sharedRegion; }

private:
    JSharedBusMemory* m_sharedRegion{nullptr};
