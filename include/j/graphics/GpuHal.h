 ...
 */
enum class JGpuApiType : uint32_t {
    Vulkan,
    Metal,
    Software
};
 ...
 */
struct JNativeWindowHandle {
    JGpuApiType apiTarget;
...
class JGpuHal {
public:
    virtual ~JGpuHal() = default;

    // Rigid asset topology; deny shallow buffer duplicates across execution bounds
    JGpuHal(const JGpuHal&) = delete;
    JGpuHal& operator=(const JGpuHal&) = delete;
    JGpuHal(JGpuHal&&) noexcept = default;
    JGpuHal& operator=(JGpuHal&&) noexcept = default;

    /**
     * @brief Factory method to initialize a hardware rendering subsystem instance.
     */
    static std::unique_ptr<JGpuHal> create(JGpuApiType api, const JNativeWindowHandle& windowHandle);
     ...
     */
    virtual JGpuFrameContext beginFrame(GpuSurfaceId sid = kPrimarySurface) = 0;

    /**
     * @brief Upload a greyscale R8 font atlas bitmap as a GPU texture.
     * Call once after JFontEngine::buildAtlas() and before the first frame.
     */
     ...
     */
    virtual void drawPrimitives(const JPrimitiveBuffer& buffer) = 0;

    /**
     * @brief Dispatches recorded operations directly to physical hardware execution paths.
     */
    virtual void submitAndPresentFrame(const JGpuFrameContext& context) = 0;
    ...
    virtual JGpuApiType getBackendType() const noexcept = 0;
